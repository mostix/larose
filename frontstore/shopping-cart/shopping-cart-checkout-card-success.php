...
//  echo "<pre>";print_r($_SESSION);echo "</pre>";exit;
  ...
  //if(true) {
    ...
    $query_order_trans = "SELECT `order_id`,`reference`, `trans_id`, `auth_code`, `response_code`, `result` 
                            FROM `order_transaction` 
                           WHERE `payment_id` = '$payment_id'";
    $result_order_trans = mysqli_query($db_link, $query_order_trans);
    if(!$result_order_trans) echo mysqli_error($db_link);
    if(mysqli_num_rows($result_order_trans) > 0) {
      $order_trans_row = mysqli_fetch_assoc($result_order_trans);
      
      $order_id = $order_trans_row['order_id'];
      $reference = $order_trans_row['reference'];
      $trans_id = $order_trans_row['trans_id'];
      $auth_code = $order_trans_row['auth_code'];
      $response_code = $order_trans_row['response_code'];
      $payment_result = $order_trans_row['result'];
    }
...
?>
      <h1 class="error" style="margin: 10px 0 20px;padding: 0;"><?=$languages[$current_lang]['text_order_canceled_01'];?></h1>
<?php
    }
    $message_output = "";
