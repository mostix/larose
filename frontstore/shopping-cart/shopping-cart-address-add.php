<?php
  ...
  if(isset($_POST['insert_customer_address_bg'])) {
    ...
    foreach($_POST as $name => $value) {
      ...
      if(empty($trimed_value) && ($name != "insert_customer_address_bg" && $name != "customer_address_site_name_label" && $name != "customer_address_site_id" && $name != "customer_address_info" && $name != "customer_address_is_default")) {
...
?>
    <form name="add_customer_address" method="post" id="bg_form" class="form form-horizontal" action="/<?=$_GET['page'];?>">
      ...
      <div>
        ...
        <a href="/<?=$current_lang;?>/shopping-cart/shopping-cart-addresses" class="btn btn-primary btn-primary button outline-outward">
